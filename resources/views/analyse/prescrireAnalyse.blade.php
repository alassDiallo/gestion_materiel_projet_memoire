...
<div>
<h4  class="text-center">Analyse</h4><hr/>
<div id="contenu">
</div>
...
<Form action="{{ route('analyse.store') }}" method="POST">
   @csrf
    <div>
    <input type="hidden" name="idAnalyse" id="idAnalyse" />
...
<script>
    ...
    //  })
     ...
     $('#idAnalyse').attr('value',$('#idAnalyse').val());
...
//   else{
  ...
  console.log($('#form').serialize());
    //console.log($('#idordonnance').val());
        $.ajax({
         url:"{{  route('analyse.store') }}",
         method:"POST",
         data:$('#form').serialize(),
         success:function(data){
             console.log(data);
            var div="<div class='row'><h4 class='col-md-6'>"+data.libelle+"</h4><span class='col-md-3'>prix : "+data.prix+"</span></div><hr/>";
            $('#contenu').append(div);
            $('#total').attr('value',(parseInt($('#total').attr("value")))+(data.prix));
